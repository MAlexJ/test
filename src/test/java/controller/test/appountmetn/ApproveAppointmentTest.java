...
public class ApproveAppointmentTest {
   ...
   public void testApproveAppointment() {
      ...
      try {
         actualResponse = Unirest.post(URL_APPROVE_APPOINTMENT)
      ...
      String appointments = jsonObjectActual.get("appointments").toString();

      List<AppointmentVO> actualAppointmentList = new Gson().fromJson(appointments, new TypeToken<ArrayList<AppointmentVO>>() {
      }.getType());
      ...
      assertEquals(actualAppointment.getPatientName(), expectAppointment.getPatientName());
      assertEquals(actualAppointment.getPatientImageUrl(), expectAppointment.getPatientImageUrl());
   }
